...
class LlamaEvaluator:
        ...
        try:
            ...
            if self.device == "mps":
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, torch_dtype=torch.float16, device_map=None
                )
                self.model.to(self.device)
            else:
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, torch_dtype=torch.float32, device_map=None
                )
                self.model.to(self.device)
    ...
    async def evaluate_responses(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        try:
            ...
            rankings = [
                Ranking(llm_name=r["llm_name"], score=r["score"])
    ...
    def _parse_evaluation(self, eval_text: str) -> Dict:
        ...
        try:
            ...
            result = {"criteria_scores": {}, "reasoning": "", "rankings": []}
            ...
            if not result["rankings"]:
                for llm_name, scores in result["criteria_scores"].items():
                    if scores:
                        ...
                        result["rankings"].append(
                            {"llm_name": llm_name, "score": round(float(avg_score), 2)}
                        )
            ...
            for eval_section in evaluations[1:]:  # Skip first empty split
                try:
                    ...
                    if "SCORES:" in eval_section:
                        scores_section = eval_section.split("SCORES:")[1].split(
                            "REASONING:"
                        )[0]
                        scores = {}
                    ...
                    if "REASONING:" in eval_section:
                        reasoning = eval_section.split("REASONING:")[1].split(
                            "EVALUATION FOR"
                        )[0]
                ...
                for line in ranking_section.split("\n"):
                    if ":" in line and any(char.isdigit() for char in line):
                        try:
                            ...
                            logger.error(
                                f"Error parsing ranking line '{line}': {str(e)}"
                            )
                            continue
...
async def health_check():
    ...
    if llama_evaluator.device == "mps":
        memory_info = (
            f", MPS Memory: {torch.mps.current_allocated_memory() / 1024**2:.2f} MB"
        )
